...
I have argued there that the metascientific advantage is to validate the induction process and the practical advantage is to increase productivity.
...
How likely is it that an analysis created with `repro` may be still reproducible after decades?
...
Hence, one needs some sort of storage provider which saves the container image and the other files associated with the project.
This problem of longterm storage for scientific data is widely recognized and because of the limited requirements---a typical project (except data) needs around one gigabyte of storage---there are several solutions available [@doornIntroductionArchivingResearch2007].
When storage availability is garunted pretty much all long term reproducibility rests on the continued support of container software.
...
This diversity in possible solutions makes long term support more likely, especially because Docker puts a lot of effort into backwards compatibility [^olddocker].
However when long term support is of primary interest, one can convert the docker image to a raw disk image and also archive this. Such strategy opens two possibilities.
First, such images are supported by all standard virtualization software. 
Such reliance on platform virtualization is standard practice in disaster recovery plans of technical infrastructure [@maitraDisasterRecoveryPlanning2011].
Second, such raw disk images can be directly installed on the hardware, without virtualization layer.
...
[^ram]: The change from x32 to x64 architecture was driven by the RAM limit. x32 has an addressable memory of 4 GiB (1 Gigabyte = $2^{30}$ bit), while x64 has an addressable memory of 16 EiB (1 Exabyte  = $2^{60}$ bit). 
...
This assumptions holds for most open source software and is straight forward to implement for all software available in the Ubuntu Package Archive.
...
First, they provide a possibility for reusing the hosts Matlab license within the container.
...
A prime example of this are ethical and legal considerations of publishing data that can only with great difficulty be de-identified.
...
The same holds true for many other problems that are either field or even project specific.
While such problems are beyond the scope of this thesis, I hope that future research concentrates on field specific reproducibility problems and their solution e.g. for longtitudanal or neuroimaging data.

These general limitations of the workflow are coupled with some `repro` specific limitations.
The most difficult problem with `repro` is the tradeoff between flexibility and usability.
...
A major difficulty in this context is the unintended flexibility users get, because I decided to keep explicit specification to an absolute minimum.
Hence, `repro` needs to infer the current state of the users project and computer.
But because users have unlimited freedome to design a project as they like it is clear that `repro` can not cover all possibilities.
Therefore it is easiest to start with the provided template and customize it.
As `repro` majors and is applied in more usecases it will become more flexible with regard to the users setup.
