...
However, neither verification of the induction nor the induces results is strictly enabled by the definition of reproducibility provided by @claerboutElectronicDocumentsGive1992 given above.
...
Hence, reproducibility has a technical aspect, which is ensuring the identical results, and a non-technical side, which is facilitating understanding and progress through cumulative science.
...
This demanding standard of reproducibility is justified by two recent developments in the social sciences in general and psychology in particular: the emergence of a "replication crises" [@ioannidisWhyMostPublished2005] and the rise of "machine learning" [@jordanMachineLearningTrends2015] as a scientific tool.
...
It turns out that, if one fits the very same statistical model as published on newly gathered data, one fails more often to achieve results that are consistent with what was published then one succeeds. [@opensciencecollaborationEstimatingReproducibilityPsychological2015].
...
Such possibility enables commutative science and allows researcher to build a more complete and accurate understanding of the fields subject matter.
