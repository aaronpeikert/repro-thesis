...
That is because as a non-technical requirement, no set of rules can assure its fulfillment (though clear writing[^writing] and clean code[^clean] certainly help).
...
It enables researcher to learn by easy to understand examples e.g.how files should refer to each other.
...
A clear file structure helps researchers to better understand how a scientific result relates to the code but an otherwise strict segregation of code and document may obscure how both connect, e.g. which parts of code generate which results.
...
Because researcher are allready familiar with RMarkdown this solution provides minimal cognitive overhead.
...
While R users have some experience with interactive programms that can only be accessed through written commands (i.e. R itself), Git follows---from the perspective of R users---strange and archaic syntactical rules.
...
Because `repro` has the same interface as `usethis` the cognitive overhead is keept as low as possible.
...
The repro package provides a simplified interface to the essential features of Make, eschewing the need to learn yet another language, while leveraging its most important feature: "dependency management" for scientific data analyses. Similiarly to Git does Make use an interface that is quit different to the way R is used. Because Make doesn't have to be used interactively, `repro` reuses the interface of RMarkdown to interact with Make.
...
Furthermore, does `repro` provide the infrastructure that connects Make and Docker e.g. it adds appropriate Make target for the container's image, and tunnels the other Make targests through the container.
