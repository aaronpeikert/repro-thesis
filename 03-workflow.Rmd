...
# because it relies on user specific settings unavailable in the container
...
Because this document complies with all requirements described therein, it will function as an example to reproduce.
...
The function `automate()` creates a `Dockerfile` and a `Makefile`, which comply with all recommendations by @peikertReproducibleDataAnalysis2019.
