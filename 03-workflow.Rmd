...
This workflow can be approached from two perspectives, either as an author of a reproducible analysis or as a contributor.
...
Consequently, I include everyone with an interest in reproducing the analysis in the group of collaborators, be it a coauthor, a reviewer or the very person who originally created the analysis at an earlier point in time.

From the contributor's point of view `repro` acts as an assistance system, advising users on how to setup their computers and how to reproduce an analysis.
It is a major difficulty for untrained users to detect their systems state accurately and act correspondingly.
`repro` parses complex technical information and presents them in a straightforward way.
...
`repro` still expects the user to have some training in the use of R, (R-)Markdown, and Git---which is increasingly recognized as a standard in the Open Science and R community---but forgos training in Make and Docker.
An important guideline in designing `repro` was to make best practices easier to implement than their alternatives.
...
The structure of this chapter mirrors the workflow in Figure \@ref(fig:workflow), first from the perspective of an contributor, then from the perspective of an creator.
...
However, another more minimal example that actually contains an data analysis will be provided in the [Creation] section below.
...
If executed, this code opens a new R session and therefor, all code from here on out needs to run in the *new* session.
...
To balance the needs of long term support and usability, `repro` offers advice about what to do, but stops right before doing it.
